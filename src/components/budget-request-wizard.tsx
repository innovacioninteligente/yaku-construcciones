...
export function BudgetRequestWizard({ t, services }: { t: any; services: any[] }) {
  ...
  const renderSimpleForm = () => (
    <Card>
      <CardHeader>
        <CardTitle className='font-headline text-2xl'>{t.simple.title}</CardTitle>
          ...
          <form onSubmit={simpleForm.handleSubmit(handleFormSubmit)} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
             ...
             <FormField control={simpleForm.control} name="projectType" render={({ field }) => (
                <FormItem><FormLabel>{t.simple.projectType.label}</FormLabel><Select onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder={t.simple.projectType.placeholder} /></SelectTrigger></FormControl><SelectContent><SelectItem value="integral">{t.simple.projectType.options.integral}</SelectItem><SelectItem value="bathrooms">{t.simple.projectType.options.bathrooms}</SelectItem><SelectItem value="kitchen">{t.simple.projectType.options.kitchen}</SelectItem><SelectItem value="pools">{t.simple.projectType.options.pools}</SelectItem></SelectContent></Select><FormMessage /></FormItem>
            )} />
            <div className="grid md:grid-cols-2 gap-6">
                <FormField control={simpleForm.control} name="squareMeters" render={({ field }) => (
                    <FormItem><FormLabel>{t.simple.squareMeters.label}</FormLabel><FormControl><Input type="number" placeholder="80" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
  ...
  const renderFormSelection = () => (
    <div className='text-center'>
        <CardHeader>
            <CardTitle className="font-headline text-3xl md:text-4xl">{t.selection.title}</CardTitle>
            <CardDescription className="text-lg max-w-2xl mx-auto">{t.selection.description}</CardDescription>
        </CardHeader>
        <div className="grid md:grid-cols-2 gap-8 mt-8">
            <Card className="flex flex-col items-center justify-center p-8 text-center hover:border-primary cursor-pointer transition" onClick={() => setFormType('simple')}>
                ...
                <h3 className="font-headline text-2xl mb-2">{t.selection.simple.title}</h3>
                <p className="text-muted-foreground">{t.selection.simple.description}</p>
            </Card>
            <Card className="flex flex-col items-center justify-center p-8 text-center hover:border-primary cursor-pointer transition" onClick={() => setFormType('detailed')}>
                ...
                <h3 className="font-headline text-2xl mb-2">{t.selection.detailed.title}</h3>
                <p className="text-muted-foreground">{t.selection.detailed.description}</p>
            </Card>
        </div>
    </div>
  );
