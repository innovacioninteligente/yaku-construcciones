    ...
    <div className="space-y-6">
       <Accordion type="single" collapsible className="w-full">
         {bathroomFields.map((field, index) => {
            const watchRenovate = form.watch(`bathrooms.${index}.renovate`);
            return (
              <AccordionItem value={`bathroom-${index}`} key={field.id}>
                ...
                <AccordionContent className="space-y-6 pt-4">
                   <FormField
                    ...
                    name={`bathrooms.${index}.renovate`}
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 text-left">
                          <FormLabel className="text-base">{commonT.bathroom.renovateBathroom.label} {index + 1}</FormLabel>
